...
// RebalanceBotBalance (§4.5's nightly rebalancing job) — chosen well above the
// human default so a bot has room to make markets across every seeded card
// without a bad run of trades permanently draining it.
//
// Sized against the bonding curve, not plucked arbitrarily: seeded companies
// carry ~1-2M shares outstanding priced in the low thousands, and the sqrt
// curve (§4.1) needs a few hundred shares traded to nudge a card's rounded
// integer price by a single unit. A bot must therefore be able to move several
// hundred-to-thousand shares per trade — costing millions of currency units —
// for its trades to be visible in price at all, several times over before the
// next rebalance. Bots are excluded from the leaderboard (it ranks HUMAN users
// only, §8), so a large bot bankroll has no effect on player standings.
const BotStartingBalance int64 = 50_000_000
