...
type Ledger struct {
	...
	// tradeObserver, if set, is invoked once per committed trade so the
	// real-time push layer can broadcast a price tick. Optional and nil-safe:
	// the ledger has no dependency on any transport, and trades work with no
	// observer wired at all (e.g. in tests). Set once at startup via
	// SetTradeObserver before any trade runs, so no locking is needed here.
	tradeObserver TradeObserver
}

// TradeEvent is the after-the-fact notification a committed trade emits. Prices
// are integer currency units (§, cardDTO convention); PreviousPrice is the
// card's price immediately before this trade, Price the recomputed spot price
// after it, and Volume the (always positive) share count traded.
...
// TradeObserver receives one TradeEvent per successfully committed trade. It
// must not block or panic — it runs inline at the tail of ExecuteTrade, after
// the DB transaction has committed, so it can never affect the trade's outcome.
...
// SetTradeObserver installs the observer invoked after each committed trade.
// Call once during startup wiring, before the server begins serving requests.
func (l *Ledger) SetTradeObserver(obs TradeObserver) {
	l.tradeObserver = obs
}
