...
// createTestHolding gives a test user a starting position in a card via raw
// SQL — mirrors what migration 000009 does for the seeded news-reactive bot
// accounts, so a news-reactive test bot has inventory to sell down.
func createTestHolding(t *testing.T, pool *pgxpool.Pool, userID, cardID uuid.UUID, shares, avgCostBasis int64) {
	t.Helper()
	ctx := context.Background()
	_, err := pool.Exec(ctx,
		`INSERT INTO holdings (user_id, card_id, shares_owned, avg_cost_basis) VALUES ($1, $2, $3, $4)`,
		userID, cardID, shares, avgCostBasis,
	)
	if err != nil {
