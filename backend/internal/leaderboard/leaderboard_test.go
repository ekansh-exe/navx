...
func TestPrependGoat_AlwaysFirstAndPreservesRealEntries(t *testing.T) {
	userA := uuid.New()
	real := []Entry{{Rank: 1, UserID: userA, Username: "alice", NetWorth: 500}}

	got := PrependGoat(real)
	if len(got) != 2 {
		t.Fatalf("len(entries) = %d, want 2", len(got))
	}
	if !got[0].IsGoat || got[0].Username != goatUsername || got[0].NetWorthDisplay != goatNetWorthDisplay {
		t.Fatalf("entries[0] = %+v, want the GOAT tribute row", got[0])
	}
	if got[1] != real[0] {
		t.Fatalf("entries[1] = %+v, want the untouched real entry %+v", got[1], real[0])
	}
}

func TestPrependGoat_EmptyInputStillYieldsGoat(t *testing.T) {
	got := PrependGoat(nil)
	if len(got) != 1 || !got[0].IsGoat {
		t.Fatalf("entries = %+v, want exactly [GoatEntry()]", got)
	}
}
